...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for project in projects.all().order_by('created_at'):
            self.git_cmd = GitCommandLineInterface(project)
            self.git_cmd.clone()
            total_prs = 0
            total_fail_prs = 0
            #for pull_request in project.pull_requests.filter(closed_at__isnull=False, merged=False,
            #                                                 first_pairwise_conflicts__isnull=True,
            #                                                 second_pairwise_conflicts__isnull=True)\
            #        .order_by('opened_at'):
            for pull_request in project.pull_requests.filter(closed_at__isnull=False,
                                                             first_pairwise_conflicts__isnull=True,
                                                             second_pairwise_conflicts__isnull=True)\
                    .order_by('opened_at'):
                total_prs += 1
                for another_pull_request in project.pull_requests.filter(closed_at__isnull=False,
